            ...
            <div class="panel-body">
                <form action="<?php echo base_url() ?>admin/settings/add_language" method="post" class="form-inline">
                    <div class="pull-right" style="margin-right: 5px;">
                        ...
                        <button type="submit" id="add-translation" class="btn btn-dark"><?= lang('add_translation') ?></button>
                    </div>
                </form>
            </div>
        </div>
...
<?php else : ?>
    ...
    <form method="post" action="<?php echo base_url() ?>admin/settings/set_translations/<?php echo $current_languages . '/' . $language_files; ?>" class="form-horizontal" enctype="multipart/form_data" >
        ...
        <section class="panel panel-default">
            <header class="panel-heading  "> <i class="fa fa-cogs"></i>
                <?php
                $total = count($english);
                $translated = 0;
                ...
                ?>
                <?= lang('translations') ?> | <a style="color: red" href="<?= base_url() ?>admin/settings/view_translations/<?= $current_languages ?>"><?= ucwords(str_replace("_", " ", $current_languages)) ?></a> | <?= $percent ?>% <?= mb_strtolower(lang('done')) ?>
            ...
            <div class="table-responsive">
                <table class="table table-striped DataTables " id="Transation_DataTables">
                    <thead>
                        <tr>
                            <th>English</th>
                            <th class="col-xs-8"><?= ucwords(str_replace("_", " ", $current_languages)) ?></th>
                        </tr>
                    </thead>
                    <tbody>
