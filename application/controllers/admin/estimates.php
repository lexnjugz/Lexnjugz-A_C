...
class Estimates extends Admin_Controller {
    ...
    public function index($action = NULL, $id = NULL, $item_id = NULL) {
        ...
        $data['all_estimates_info'] = $this->estimates_model->get();
    ...
    public function save_estimates($id = NULL) {
        ...
        if (!empty($id)) {
            ...
            $action = lang('activity_estimates_updated');
            $msg = lang('estimate_updated');
        } else {
            $estimates_id = $this->estimates_model->save($data);
            $action = lang('activity_estimates_created');
        ...
        if (!empty($quantity)) {
            foreach ($quantity as $key => $value) {
                ...
                if (!empty($id)) {
                    ...
                    $this->estimates_model->save($data, $id);
                    $action = lang('activity_estimates_items_updated');
                } else {
                    $estimate_items_id = $this->estimates_model->save($data);
                    $action = lang('activity_estimates_items_added');
                }
                $activity = array(
                    'user' => $this->session->userdata('user_id'),
                    'module' => 'estimates',
    ...
    public function delete($action, $estimates_id, $item_id = NULL) {
        ...
        $activity = array(
            ...
            'module' => 'estimates',
            'module_field_id' => $estimates_id,
            'activity' => lang('activity_' . $action),
            'icon' => 'fa-circle-o',
            'value1' => $action
        );
    ...
    public function send_estimates_email($estimates_id) {
        ...
        $activity = array(
            ...
            'module' => 'estimates',
            'module_field_id' => $estimates_id,
            'activity' => lang('activity_estimates_sent'),
            'icon' => 'fa-envelope',
            'value1' => $ref
        );
        $this->estimates_model->_table_name = 'tbl_activities';
    ...
    public function convert_to_invoice($id) {
        ...
        $activity = array(
            ...
            'module' => 'estimates',
            'module_field_id' => $id,
            'activity' => lang('activity_estimate_convert_to_invoice'),
            'icon' => 'fa-laptop',
            'value1' => $ref
        );
        $this->estimates_model->_table_name = 'tbl_activities';
