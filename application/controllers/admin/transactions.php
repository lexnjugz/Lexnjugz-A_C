...
class Transactions extends Admin_Controller {
    ...
    public function deposit($id = NULL) {
        ...
        if (!empty($id)) {
            $data['active'] = 2;
            $data['deposit_info'] = $this->transactions_model->check_by(array('transactions_id' => $id), 'tbl_transactions');
        } else {
            $data['active'] = 1;
        }
    ...
    public function save_deposit($id = NULL) {
        ...
        if (!empty($id)) {
            $activity = lang('activity_update_deposit');
            $msg = lang('update_a_deposit');
        } else {
            $activity = lang('activity_new_deposit');
    ...
    public function delete_deposit($id) {
        ...
        $activity = lang('activity_delete_deposit');
    ...
    public function expense($id = NULL) {
        ...
        if (!empty($id)) {
            $data['active'] = 2;
            $data['expense_info'] = $this->transactions_model->check_by(array('transactions_id' => $id), 'tbl_transactions');
        } else {
            $data['active'] = 1;
        }
    ...
    public function save_expense($id = NULL) {
        ...
        if (!empty($data['amount'])) {
            if ($account_info->balance < $data['amount']) {
                $type = 'error';
                ...
                $this->transactions_model->save($data, $id);
                if (!empty($id)) {
                    $activity = lang('activity_update_expense');
                    $msg = lang('update_a_expense');
                } else {
                    $activity = lang('activity_new_expense');
                    $msg = lang('save_new_expense');
                }
                // save into activities
                $activities = array(
                    'user' => $this->session->userdata('user_id'),
                    'module' => 'transactions/expense',
                    'module_field_id' => $id,
                    'activity' => $activity,
                    'icon' => 'fa-coffee',
                    'value1' => $account_info->account_name,
    ...
    public function delete_expense($id) {
        ...
        $activity = lang('activity_delete_expense');
    ...
    public function transfer($id = NULL) {
        ...
        if (!empty($id)) {
            $data['active'] = 2;
            $data['transfer_info'] = $this->transactions_model->check_by(array('transfer_id' => $id), 'tbl_transfer');
        } else {
            $data['active'] = 1;
        }
    ...
    public function save_transfer($id = NULL) {
        ...
        if ($to_account_id == $from_account_id) {
            ...
            if ($amount > $from_acc_info->balance) {
                ...
                $type = 'success';
                if (!empty($id)) {
                    $activity = lang('activity_update_transfer');
                    $msg = lang('update_a_transfer');
                } else {
                    $activity = lang('activity_new_transfer');
                    $msg = lang('save_new_transfer');
                }
                // save into activities
                $activities = array(
                    'user' => $this->session->userdata('user_id'),
                    'module' => 'transactions/transfer',
                    'module_field_id' => $id,
                    'activity' => $activity,
                    'icon' => 'fa-coffee',
    ...
    public function delete_transfer($id) {
        ...
        $activity = lang('activity_delete_transfer');
